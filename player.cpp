...
//播放当前桌面屏幕: libvlc_media_new_location (inst, "screen://");
    ...
    /* Create a new libvlc player 创建一个播放器对象 */
    ...
    LONG style = GetWindowLong((HWND)videoWidget->winId(), GWL_STYLE );
    if( !(style & WS_CLIPCHILDREN) )
        SetWindowLong((HWND)videoWidget->winId(), GWL_STYLE, style | WS_CLIPCHILDREN );
    //设置给予媒体播放器媒体输出的win32/win64窗口句柄
    libvlc_media_player_set_hwnd(vlcPlayer, (HWND)videoWidget->winId());
#endif

    /* And start playback 开始播放*/
